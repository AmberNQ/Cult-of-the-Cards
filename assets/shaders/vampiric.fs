...
extern PRECISION vec2 vampiric;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    // red tint
    vec3 red = vec3(255., 84., 71.) / 255.;
    vec3 new_color = pixel.rgb * 0.5 + red * 0.5;
    pixel = vec4(new_color, pixel.a);

    // blood along the top
    float x_chunk = floor(floored_uv.x * 32.) / 32.;
    float line = ((1. - sin(8 * uv.x)) * (-0.1 - cos(8 * uv.x)) * (-sin(8 * uv.x)) + 2.) / 8.;
    if (floored_uv.y < line + (pow(rand(vec2(x_chunk)), 8.))) {
        pixel.rgb = vec3(189., 13., 13.) / 255.;
    }
