...
extern PRECISION vec2 merciless;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    // colors
    vec3 color1 = vec3(82., 59., 255.) / 255.;
    vec3 color2 = vec3(255., 59., 199.) / 255.;
    float position = (uv.x + uv.y) / 2.;
    vec3 new_color = color1 * (1 - position) + color2 * position;
    vec3 old_color = pixel.rgb;
    pixel = vec4(0.5 * new_color + 0.5 * old_color, pixel.a);
    ...
    number fac = 0.8 + 0.9*sin(11.*uv.x+4.32*uv.y + merciless.r*12. + cos(merciless.r*5.3 + uv.y*4.2 - uv.x*4.));
    number fac2 = 0.5 + 0.5*sin(8.*uv.x+2.32*uv.y + merciless.r*5. - cos(merciless.r*2.3 + uv.x*8.2));
    number fac3 = 0.5 + 0.5*sin(10.*uv.x+5.32*uv.y + merciless.r*6.111 + sin(merciless.r*5.3 + uv.y*3.2));
    number fac4 = 0.5 + 0.5*sin(3.*uv.x+2.32*uv.y + merciless.r*8.111 + sin(merciless.r*1.3 + uv.y*11.2));
    number fac5 = sin(0.9*16.*uv.x+5.32*uv.y + merciless.r*12. + cos(merciless.r*5.3 + uv.y*4.2 - uv.x*4.));
    ...
    for (float i = 1; i > 0.; i -= 0.25) {
        vec2 timed_uv = vec2(floored_uv.x + sin(merciless.y * 0.5) * 0.025, floored_uv.y + merciless.y * 0.025);
        float star_scale = 4. - i;
        vec2 cell_uv = fract((timed_uv + i * rand(vec2(i))) * star_scale) - 0.5;
        vec2 cell_id = floor((timed_uv + i * rand(vec2(i))) * star_scale);
        vec2 offset = vec2(rand(cell_id), rand(cell_id * 2.4868)) * 0.7 - 0.35;
        vec2 star = cell_uv + offset;
        if (abs(star.y) <= -1. * pow(abs(star.x), rand(cell_id) * 0.07) + 0.9) {
            pixel.rgb = (1.2 - i) * pixel.rgb + (i - 0.2) * vec3(1., 1., 0.8);
        }
    }
